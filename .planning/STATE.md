...
- [Found during 05 verification] `tests/pages/week.test.ts` is entirely `describe.skip` — **all 18 "skipped" tests in the suite are this one file**, so the green 385/18/0 result contains zero executing page-level tests. The pick → `picks` ref → `computed` → sidebar DOM chain is unexercised, which is why STAND-02 lands in UAT rather than passing automatically. Un-skipping likely needs the `nuxt`-environment vitest project via `@nuxt/test-utils` `defineVitestProject`
