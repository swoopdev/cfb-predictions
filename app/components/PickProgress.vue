...
const progressData = computed(() => {
  ...
  // Fallback to unfiltered season progress from composable
  return progressOverall.value
})

const percentage = computed(() => {
  if (progressData.value.total === 0) return 0
  return (progressData.value.picked / progressData.value.total) * 100
})
</script>
  ...
  <div class="relative w-full h-6 bg-muted rounded overflow-hidden">
    ...
    <div class="absolute inset-0 flex items-center justify-center">
      <span class="text-xs font-semibold text-white">
        {{ progressData.picked }}/{{ progressData.total }} picked
      </span>
    </div>
  </div>
</template>
